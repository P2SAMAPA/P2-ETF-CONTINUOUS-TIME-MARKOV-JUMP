...
st.set_page_config(page_title="Continuous‑Time Markov Jump Process", layout="wide")
...
st.markdown('<h1 style="text-align: center;">📈 Continuous‑Time Markov Jump Engine</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Macro‑dependent jump intensity to high‑volatility regime</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 Markov Jump")
...
st.sidebar.markdown(f"**High vol threshold:** {config.HIGH_VOL_THRESHOLD}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'markov_jump_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Jump intensity: {etf['jump_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized Intensity"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized Intensity", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("📈 Top ETFs by Jump Intensity to High‑Vol Regime (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Continuous‑time Markov jump process** models transitions between normal and high‑volatility states.
        - The jump intensity λ(t) = exp(β₀ + β·macro(t)) depends on current macro conditions (VIX, DXY, yields).
        - We estimate a logistic regression where the target is whether the absolute return exceeds a threshold.
        - The score is the predicted probability of a high‑volatility day, conditional on macro.
        - High score → ETF likely to experience a large move (jump) given today's macro.
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the jump intensities.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Continuous‑Time Markov Jump | Macro‑dependent jump intensity")
