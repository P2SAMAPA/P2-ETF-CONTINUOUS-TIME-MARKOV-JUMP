...
from markov_jump import continuous_time_markov_jump_score
...
def run_for_window(returns, macro_df, window_days):
    ...
    for ticker in ret_window.columns:
        s = continuous_time_markov_jump_score(
            ret_window[ticker],
            macro_window,
    ...
    top_etfs = [{"ticker": t, "jump_score_norm": s, "raw_score": raw_scores[t]} for t, s in sorted_norm[:config.TOP_N]]
    return {
        "window": window_days,
        "top_etfs": top_etfs,
...
def main():
    ...
    if macro_df is None:
        print("Warning: No macro data found. Proceeding without macro (using only intercept).")
    results = {
    ...
    out_file = f"output/markov_jump_{timestamp}.json"
